...
{
  class MapParser : IResponseParser, IAggregateResponseParser
  {
    private byte _lastByte = 0x00;
    private int _keyValuePairCount = 0;
      ...
      while (currentPosition <= lastBytePosition)
      {
        ...
        if (currentByte == 0x0A)
        {
          if (_lastByte == 0x0D)
          {
            ...
            Result = new RespMap(_keyValuePairCount);
            return Result;
          }
          else
          {
        ...
        if (currentByte != 0x0D)
        {
          _keyValuePairCount = 10 * _keyValuePairCount + currentByte - '0';
